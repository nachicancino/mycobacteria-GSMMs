...
ref = '/home/icancino/Documents/Models/Ref/modified_H37Rvfast501e_10.xml'
...
for genome in genome_list:
    if Path(str(new_dir_cmd)+'/'+(str(genome).split('.')[0]+'_new')).exists():
        os.chdir(str(new_dir_cmd)+'/'+str(genome).split('.')[0])
        print('Repository exists')
    else:
        print('Creating rep. '+str(genome).split('.')[0])
        os.mkdir(str(new_dir_cmd)+'/'+str(genome).split('.')[0]+'_new')
        os.chdir(str(new_dir_cmd)+'/'+str(genome).split('.')[0]+'_new')
    with open("CARVEME_PARAMETERS.json", "w") as readme:
        for arg in args:
            for evalue in evalues:
    	        for pidentity in piden:
                    gen = dir_cmd + genome_folder + '/'+ genome
                    outname = 'new_'+ genome.split('.')[0] +str(arg)[2:]+ str(pidentity)+str(evalue)+'.xml'
                    print(gen)
